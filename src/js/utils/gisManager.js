...
module.exports = {
  ...
  *
  */
  arraysEquals: function (arr1, arr2) {
    if (arr1 === arr2) {
      return true;
    }
  ...
  arraysIntersection: function(arr1, arr2) {
    ...
    for (let arr_a of arr1) {
      for (let arr_b of arr2) {
        if (this.arraysEquals(arr_a, arr_b)) {
          result.push(arr_a);
        }
      }
    }
    // Unicité des points d'intersection
  ...
  */
  ...
  geoJsonMultiLineStringCoordsToSingleLineStringCoords: function(srcCoords) {
    ...
    for (let i = 1; i < srcCoords.length; i++) {
      ...
      if (!this.arraysEquals(dissolvedCoords[dissolvedCoords.length - 1 ], curr_line[0])) {
        curr_line.reverse();
      }
